...
struct DashboardView: View {
    ...
    // MARK: - Sections
    //
    // These are split out of `body` so each stays small enough for the Swift
    // type-checker; an inlined `body` this large times out on some machines.

    private var salesChannelSelector: some View {
        Menu {
            Button {
    ...
    private var todaysOrders: some View {
        VStack(alignment: .leading, spacing: 20) {
            HStack {
                Text("Today's orders")
